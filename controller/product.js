...
const productController = {
    ...
    async view_45_products(request,response){
        try{
            ...
            let products = await productOperations.view_45_products(parseInt(skip),parseInt(limit),rating);
            if(products){
    ...
    async view_by_interest(request,response){
        try{
            ...
            let products = await productOperations.find_by_interest(doc.interested_in,rating);
            if(products){
    ...
    async view_by_categories(request,response){
        try{
            ...
            let products =await productOperations.view_by_categories(categories,rating);
            if(products){
                response.status(SUCCESS).json({message:messageBundle["product.found"],AllProducts:products});
            }
            else{
    ...
    async view_by_name_category(request,response){
        try{
            ...
            let products = await productOperations.view_by_name_category(name,category,rating);
            if(products){
                response.status(SUCCESS).json({message:messageBundle["product.found"],AllProducts:products});
            }
            else{
    ...
    async view_by_name_categories(request,response){
        try{
            ...
            let products = await productOperations.view_by_name_categories(name,categories,rating);
            if(products){
                response.status(SUCCESS).json({message:messageBundle["product.found"],AllProducts:products});
            }
            else{
    ...
    async view_by_price_categories(request,response){
        try{
            ...
            let lt=request.query.lt;
            let categories = request.body.categories;
            let products = await productOperations.view_by_price_categories(gt,lt,categories);
            if(products){
                response.status(SUCCESS).json({message:messageBundle["product.found"],AllProducts:products});
            }
            else{
    ...
    async view_by_product_id(request,response){
        try{
            ...
            if(footwear){
                response.status(SUCCESS).json({message:messageBundle["product.found"],footwear:footwear});
            }
            else{
                response.status(NOT_FOUND).json({message:messageBundle["product.notfound"]});
            }
    ...
    async rate_product(request,response){
        try{
            ...
            let check = await orderOperations.check_order_status(doc.user_id,product_id);
            if(check){
                let check_rated = await orderOperations.check_rated(doc.user_id,product_id);
                if(check_rated){
                    let product =  await productOperations.view_by_product_id(product_id);
                    let rated_by=product.rated_by + 1;
                    let new_rating = ((product.product_rating*product.rated_by)+rating)/rated_by;
                    let update = await productOperations.update_rating(product_id,new_rating,rated_by);
    ...
    async edit_rating(request,response){
        try{
            ...
            let product_id = request.body.product_id;
            let rating = request.body.rating;
            let obj =await orderOperations.view_order_by_product_id(doc.user_id,product_id);
            let product = await productOperations.view_by_product_id(product_id);
            if(obj){
                let product_rating = product.product_rating;
                let old_rating = obj.product_rated;
                let rated_by = product.rated_by;
                let new_rating = ((product_rating*rated_by)+rating-old_rating)/rated_by;
                let change_rated = await orderOperations.product_rated(doc.user_id,product_id,new_rating);
                let change =await productOperations.update_rating(product_id,new_rating,rated_by);
    ...
    async review_product(request,response){
        try{
            ...
            let check = await orderOperations.check_order_status(doc.user_id,product_id);
            if(check){
                let check_reviewed = await productOperations.check_reviewed(doc.user_id,product_id);
                if(!check_reviewed){
    ...
    async edit_review(request,response){
        try{
            ...
            let product_id = request.body.product_id;
            let review = request.body.review;
            let check = await productOperations.check_reviewed(doc.user_id,product_id);
            if(check){
                let update_review = await productOperations.update_review(doc.user_id,product_id,review);
    ...
    async search_in_reviews(request,response){
        try{
            ...
            let reviews = await productOperations.search_in_reviews(product_id,search);
            if(reviews){
                response.status(SUCCESS).json({message:messageBundle['found'],reviews:reviews});
            }
            else{
